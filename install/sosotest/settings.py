...
AUTH_LDAP_BIND_PASSWORD = 'YmvZuB]mNza,RoG*3p+6'
AUTH_LDAP_USER_SEARCH = LDAPSearchUnion(
    LDAPSearch('ou=链家网,dc=ljstaff,dc=com',  ldap.SCOPE_SUBTREE, "(&(objectClass=person)(sAMAccountName=%(user)s))"),
    LDAPSearch('ou=链家,dc=ljstaff,dc=com', ldap.SCOPE_SUBTREE, "(&(objectClass=person)(sAMAccountName=%(user)s))"),
    LDAPSearch('ou=贝壳,dc=ljstaff,dc=com', ldap.SCOPE_SUBTREE, "(&(objectClass=person)(sAMAccountName=%(user)s))"),
    LDAPSearch('ou=新房平台,dc=ljstaff,dc=com', ldap.SCOPE_SUBTREE, "(&(objectClass=person)(sAMAccountName=%(user)s))"),
    LDAPSearch('ou=经纪平台,dc=ljstaff,dc=com', ldap.SCOPE_SUBTREE, "(&(objectClass=person)(sAMAccountName=%(user)s))"),
    LDAPSearch('ou=交易平台,dc=ljstaff,dc=com', ldap.SCOPE_SUBTREE, "(&(objectClass=person)(sAMAccountName=%(user)s))"),
    LDAPSearch('ou=租赁平台,dc=ljstaff,dc=com', ldap.SCOPE_SUBTREE, "(&(objectClass=person)(sAMAccountName=%(user)s))"),
    LDAPSearch('ou=楼盘字典,dc=ljstaff,dc=com', ldap.SCOPE_SUBTREE, "(&(objectClass=person)(sAMAccountName=%(user)s))"),
)

AUTH_LDAP_USER_ATTR_MAP = {"first_name": "cn","email": "mail"}
AUTH_LDAP_ALWAYS_UPDATE_USER = True

AUTH_LDAP_GROUP_SEARCH = LDAPSearch("OU=mailground,DC=ljstaff,DC=com",ldap.SCOPE_SUBTREE, "(objectClass=group)" )  #搜>

...
from runfunc.initial import TcpServerConf

class DubboTask(DubboTaskBase):
    """
    任务表，包括任务执行，报告生成等。
    """

    def __init__(self):
        super(DubboTask, self).__init__()
    ...
    def execute(self):
        ...
        """
        logging.debug(
            "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$任务执行ID[%d]任务ID[%s]开始执行在环境httpConfKey[%s]执行人[%s]$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$" % (
            int(self.taskExecuteId), self.taskId, self.httpConfKey, self.addBy))
        ...
        if int(self.taskSuiteExecuteId) > 0:
            ...
            self.serviceRedis.set_data(
                "%s_taskSuite_%s_task_%s" % (self.protocol, self.taskSuiteExecuteId, self.taskExecuteId),
                json.dumps(redisValue), 60 * 60 * 12)
    ...
    def generateByTaskExecuteId(self):
        ...
        sqlTasExec = "select * from tb2_dubbo_task_execute where id=%d" % int(self.taskExecuteId)
        ...
        if resTaskExec:
            ...
            if self.version == "CurrentVersion":
                ...
                for tmpCaseId in caseIdList:
                    getCaseListSql += "select * from tb2_dubbo_testcase where state = 1 and caseId='%s' union all " % tmpCaseId
                ...
                sqlCaseSteps = "select * from tb2_dubbo_testcase_step where state = 1 and caseId in (%s) order by stepNum asc" % caseListForIn
                ...
                for tmpInterfaceId in interfaceIdList:
                    getInterfaceListSql += " SELECT * FROM tb2_dubbo_interface where state = 1 and interfaceId = '%s' union all " % (
                    tmpInterfaceId)
                ...
                getCaseListSql = ""
                for tmpCaseId in caseIdList:
                    getCaseListSql += "select * from tb_version_http_testcase where state = 1 and caseId='%s' and versionName='%s' union all " % (
                    tmpCaseId, self.version)
                getCaseListSql = getCaseListSql[:-11]
                resCaseListData = self.globalDB.execute_sql(getCaseListSql)
                ...
                for tCid in caseIdList:
                    caseListForIn += "'%s'," % tCid
                caseListForIn = caseListForIn[:-1]
                sqlCaseSteps = "select * from tb_version_http_testcase_step where state = 1 and caseId in (%s)  and versionName='%s' order by stepNum asc" % (
                caseListForIn, self.version)
                resCaseStepData = self.globalDB.execute_sql(sqlCaseSteps)
                resCaseStepDict = {}
                for tmpCaseStep in resCaseStepData:
                    if tmpCaseStep['caseId'] not in resCaseStepDict.keys():
    ...
    def generateHttpReport(self):
        ...
        if os.path.exists(dateFolder) == False:
            os.mkdir(dateFolder)
        fileName = "DUBBO_%s_%s_%s.html" % (self.taskExecuteId, self.taskId, get_current_time_YYYYMMDDHHMMSS())
        self.testReportFilePath = "%s/%s" % (dateFolder, fileName)
        if DubboReport().createHttpReportByTask(self, self.testReportFilePath):
            ...
            if self.runTaskServiceIp != TcpServerConf.ip:
                ...
                if sendFtp(TcpServerConf.ip, "setAPITaskReports", "setAPITaskReports", dateFolderName, fileName,
                ...
                print("当前执行机为主服务器，不执行传输操作",self.runTaskServiceIp)
                logging.info("当前执行机为主服务器，不执行传输操作")
            return True
        else:
            return False
    ...
    def updateExecResultsByTaskExecuteId(self):
        ...
        sqlTasExec = "UPDATE tb2_dubbo_task_execute SET highPriorityVARS='%s',execTime='%s',execFinishTime = '%s' ,execTakeTime = %d ,testResult = '%s',testResultMsg = '%s',testReportUrl = '%s' where id=%d" \
        ...
        if self.globalDB.execute_sql(sqlTasExec) == False:
            sqlTasExec2 = "UPDATE tb2_dubbo_task_execute SET execTime='%s',execFinishTime = '%s' ,execTakeTime = %d ,testResult = '%s',testResultMsg = '%s',testReportUrl = '%s' where id=%d" \
                          % (self.execTime,
                             self.execFinishTime,
                             self.taskExecTakeTime,
                             replacedForIntoDB(self.testResult),
    ...
    def updateExecuteStatusToDB(self):
        ...
        """
        self.globalDB.initGlobalDBConf()
        sqlTasExec = "UPDATE tb2_dubbo_task_execute SET execStatus = %d where id=%d" % (
        int(self.execStatus), int(self.taskExecuteId))
    ...
    def updateExecuteStartTime(self):
        ...
        sqlTasExec = "UPDATE tb2_dubbo_task_execute SET execTime='%s' where id=%d" % (self.execTime, int(self.taskExecuteId))
    ...
    def insertInterfaceInfosToDB(self):
        ...
        for tmpKey, tmpValue in self.totalInterfaceCountDict.items():
            interfaceUrl = tmpKey
            requestHost = tmpValue['requestHost']
            ...
            if requestHost.strip() != "":
                tmpsql = """INSERT INTO tb2_dubbo_interface_execute_history(interfaceUrl,requestHost,totalCount,passCount,failCount,errorCount,exceptionCount,taskExecuteId,taskId,title,taskDesc,protocol,httpConfKey,execBy,testReportUrl,addBy,modBy,addTime,modTime,state) 
                         VALUES("%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s",%d)""" % \
                         (replacedForIntoDB(interfaceUrl), replacedForIntoDB(requestHost), totalCount, passCount,
                          failCount, errorCount, exceptionCount, taskExecuteId, taskId, replacedForIntoDB(title),
                          replacedForIntoDB(taskDesc),
                          protocol, httpConfKey, execBy, replacedForIntoDB(testReportUrl), addBy, modBy,
    ...
    def sendEmailToExecutor(self):
        ...
        resEmailList = self.globalDB.execute_sql(
            "select emailList from tb2_dubbo_task_execute where id=%d" % int(self.taskExecuteId))
        self.globalDB.release()
        if res:
            emailList = "%s" % res[0]['email']  # ;分号间隔
            ...
            if resEmailList:
                ...
                emailList = emailList
            logging.info("EMAIL LIST:%s" % emailList)
...
接口[/json/crm_common_detail/lock-data.action]：已执行1次，通过0，失败1，通过率0.00%。
 详情见附件。
            ...
            self.taskId, self.addBy,self.confHttpLayer.alias, self.confHttpLayer.key, self.execTime, self.testResult,
            self.taskExecTakeTime, self.title, self.taskDesc, CommonConf.groupLevel1, self.businessLineGroup,
            CommonConf.groupLevel2, self.modulesGroup,
            (self.execComments.strip() == "" and "无" or self.execComments.strip())))
            if isDictJson(self.testResultMsg):
                ...
                if resDict['totalExecuteSummary']['total'] > 0:
                    emailText = emailText + """<tr><th align="left" width="20%%">所有接口统计</th><td width="80%%">接口总数%d个，已执行接口%d个，通过%d，失败%d，未执行%d个，通过率%.2f%%。</td></tr>""" \
                                            % (resDict['interfaceExecuteSummary']['total'] +
                                               resDict['testcaseStepExecuteSummary']['total'],
                                               (resDict['interfaceExecuteSummary']['total'] +
                                                resDict['testcaseStepExecuteSummary']['total'] -
                                                resDict['interfaceExecuteSummary']['notrun'] -
                                                resDict['testcaseStepExecuteSummary']['notrun']),
                                               resDict['interfaceExecuteSummary']['pass'] +
                                               resDict['testcaseStepExecuteSummary']['pass'],
                                               resDict['interfaceExecuteSummary']['fail'] +
                                               resDict['testcaseStepExecuteSummary']['fail'],
                                               resDict['interfaceExecuteSummary']['notrun'] +
                                               resDict['testcaseStepExecuteSummary']['notrun'],
                                               ((float(resDict['interfaceExecuteSummary']['pass'] +
                                                       resDict['testcaseStepExecuteSummary']['pass']) / float(
                                                   resDict['interfaceExecuteSummary']['total'] +
                                                   resDict['testcaseStepExecuteSummary']['total'])) * 100))
                if resDict['interfaceExecuteSummary']['total'] > 0:
                    emailText = emailText + """<tr><th align="left">独立接口统计</th><td>已执行接口%d个，通过%d，失败%d，通过率%.2f%%。</td></tr>""" % (
                    resDict['interfaceExecuteSummary']['total'], resDict['interfaceExecuteSummary']['pass'],
                    resDict['interfaceExecuteSummary']['fail'], ((float(
                        resDict['interfaceExecuteSummary']['pass']) / float(
                        resDict['interfaceExecuteSummary']['total'])) * 100))
                if resDict['testcaseExecuteSummary']['total'] > 0:
                    emailText = emailText + """<tr><th align="left">用例统计</th><td>已执行用例%d个，通过%d，失败%d，通过率%.2f%%。</td></tr>""" % (
                    resDict['testcaseExecuteSummary']['total'], resDict['testcaseExecuteSummary']['pass'],
                    resDict['testcaseExecuteSummary']['fail'], ((float(
                        resDict['testcaseExecuteSummary']['pass']) / float(
                        resDict['testcaseExecuteSummary']['total'])) * 100))
                ...
                for k, v in self.businessLineExecuteSummayDict.items():
                    emailText = emailText + """<tr><th align="left">%s</th><td>接口总数%d个，已执行接口%d个，通过%d，失败%d，未执行%d个，通过率%.2f%%。</td></tr>""" % (
                        k, v['total'], (v['total'] - v['notrun']), v['pass'], v['fail'], v['notrun'],
                        ((float(v['pass']) / float(v['total'])) * 100)
                    )
                    for km, vm in self.moduleExecuteSummayDict[k].items():
                        emailText = emailText + """<tr><th align="left">%s->%s</th><td>接口总数%d个，已执行接口%d个，通过%d，失败%d，未执行%d个，通过率%.2f%%。</td></tr>""" % (
                            k, km, vm['total'], (vm['total'] - vm['notrun']), vm['pass'], vm['fail'], vm['notrun'],
                            ((float(vm['pass']) / float(vm['total'])) * 100)
                        )
                emailText = emailText + """</table>"""
                ...
                if self.testResult != ResultConst.PASS:
                    ...
                    emailText = emailText + "<h2>按照%s%s统计失败的接口和用例</h2>" % (
                    CommonConf.groupLevel1, CommonConf.groupLevel2)
                    # 列出失败的接口+===========================================================================
                    emailText = emailText + """<table border="1px" cellpadding="3px" width="80%%">"""
                    emailText = emailText + """<tr><th width="20%%" align="left">%s->%s</th><th width="20%%" align="left">接口/用例ID</th><th width="50%%" align="left">描述</th><th width="10%%" align="left">结果</th></tr>""" % (
                    CommonConf.groupLevel1, CommonConf.groupLevel2)

                    for tmpInterface in self.taskInterfaceList:
                        if tmpInterface.testResult != ResultConst.PASS:
                            emailText = emailText + ("<tr><td>%s->%s</td><td>%s</td><td>%s</td><td>%s</td></tr>" % (
                            self.busiDict[str(tmpInterface.businessLineId)], self.moduleDict[str(tmpInterface.modules)],
                            tmpInterface.interfaceId, tmpInterface.title, tmpInterface.testResult))

                    for tmpTestcase in self.taskTestcaseList:
                        if tmpTestcase.testResult != ResultConst.PASS:
                            emailText = emailText + ("<tr><td>%s->%s</td><td>%s</td><td>%s</td><td>%s</td></tr>" % (
                            self.busiDict[str(tmpTestcase.businessLineId)], self.moduleDict[str(tmpTestcase.moduleId)],
                            tmpTestcase.caseId, tmpTestcase.title, tmpTestcase.testResult))
                            for i in range(0, len(tmpTestcase.stepTestcaseList)):
                                emailText = emailText + ("<tr><td></td><td>步骤：%s</td><td>%s</td><td>%s</td></tr>" % (
                                tmpTestcase.stepTestcaseList[i].stepNum, tmpTestcase.stepTestcaseList[i].desc,
                                tmpTestcase.stepTestcaseList[i].testResult))
                    ...
                    if len(interfaceCountDict) > 0:
                        ...
                        for k, v in interfaceCountDict.items():
                            if v['total'] != v['pass']:
                                emailText = emailText + "<tr><th width=40%% align='left'>%s</th><td width=60%% >已执行%d次，通过%d，失败%d，通过率%.2f%%。</td></tr>" % (
                                k, v['total'], v['pass'], v['fail'], ((float(v['pass']) / float(v['total'])) * 100))
                    ...
                    if len(totalInterfaceCountDict) > 0 and len(interfaceCountDict) > 0 and len(
                            testcaseStepInterfaceCountDict) > 0:
            ...
            emailList = "qualitycenter@lianjia.com"  # ;分号间隔
    ...
    def getAllBusinessLine(self):
        self.globalDB.initGlobalDBConf()
        resList = self.globalDB.execute_sql("select id,bussinessLineName from tb_business_line order by id ",
                                            auto_release=True)
        retDict = {}
        for tmp in resList:
    ...
    def generateTaskExecuteSummaryDict(self):
        ...
        for i in range(0, len(self.taskInterfaceList)):
            ...
            if tmpBusi not in self.businessLineExecuteSummayDict.keys():
                self.businessLineExecuteSummayDict[tmpBusi] = {"total": 0, "pass": 0, "fail": 0, "error": 0,
                                                               "notrun": 0, "failedCaseList": []}
            ...
            if tmpModule not in self.moduleExecuteSummayDict[tmpBusi].keys():
                self.moduleExecuteSummayDict[tmpBusi][tmpModule] = {"total": 0, "pass": 0, "fail": 0, "error": 0,
                                                                    "notrun": 0, "failedCaseList": []}
            if tmpInterface.testResult == ResultConst.PASS:

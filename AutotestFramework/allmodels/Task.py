        ...
        if HttpReport().createHttpReportByTask(self,self.testReportFilePath):
            ...
            if isReleaseEnv:
                if CommonConf.http_report_to_AWS == "1":
                    try:
                        os.system("aws s3 cp %s s3://test-team/http_interface_report/" % self.testReportFilePath)
                        self.testReportUrl = "https://s3.cn-north-1.amazonaws.com.cn/test-team/http_interface_report/%s" % fileName
                    except:
    ...
    def sendEmailToExecutor(self):
        ...
        if res:
            ...
            if resEmailList:
                ...
                # emailList = emailList + ";qualitycenter@lianjia.com"

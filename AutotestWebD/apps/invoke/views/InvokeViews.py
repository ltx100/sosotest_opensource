...
#invoke入口函数
def index(request):
    global retMsg,langDict,addBy,httpConfKey,isSendEmail,isSaveHistory,execPlatform,token,timeout,host,version,testLevel,addByName,addByObj
    ...
    if opt == "gettoken":
        ...
        if opt == "runtask":
            return runtask(request, paramDict)
        elif opt == "callbacktask" :
            return callbacktask(paramDict)
        elif opt == "canceltask":
            return canceltask(paramDict)
        elif opt == "gettaskprogress":
            return gettaskprogress(paramDict)
        elif opt == "runinterface" :
            return runinterface(request,paramDict)
        elif opt == "callbackinterface" :
            return callbackinterface(paramDict)
        elif opt == "runtestcase" :
            return runtestcase(request,paramDict)
        elif opt == "callbacktestcase":
            return callbacktestcase(paramDict)
        elif opt == "runuitask":
            return runuitask(request, paramDict)
        elif opt == "callbackuitask":
            return callbackuitask(paramDict)
        elif opt == "canceluitask":
            return canceluitask(paramDict)
        elif opt == "rundubbotask":
            logger.debug("INTO rundubbotask")
            return rundubbotask(request, paramDict)
        elif opt == "callbackdubbotask":
            return callbackdubbotask(paramDict)
        elif opt == "canceldubbotask":
            return canceldubbotask(paramDict)
        elif opt == "getdubbotaskprogress":
            return getdubbotaskprogress(paramDict)
        elif opt == "addStatis":
            return addStatisticTaskExecuteInfo(paramDict)
        elif opt == "runtasksuite":
            return runtasksuite(request,paramDict)
        elif opt == "callbacktasksuite":
            return callbacktasksuite(paramDict)
        elif opt == "canceltasksuite":
            return canceltasksuite(paramDict)
        elif opt == "rundubbotasksuite":
            return rundubbotasksuite(request,paramDict)
        elif opt == "callbackdubbotasksuite":
            return callbackdubbotasksuite(paramDict)
        elif opt == "canceldubbotasksuite":
            return canceldubbotasksuite(paramDict)
        else:
            return HttpResponse(ApiReturn(ApiReturn.CODE_PARAM_ERROR, eval(langDict['invoke']['optparaminvalid']) , body=retMsg).toJson())
...
def getTaskResult(token,taskExecuteId,timeout):
    ...
    while True:
        ...
        if startTimer > timeout :
            return_msg = {}
            return_msg['callbacktaskUrl'] = "HTTP://%s/invoke?opt=callbacktask&token=%s&taskExecuteId=%s" % (host,token,taskExecuteId)
            return_msg['canceltaskUrl'] = "HTTP://%s/invoke?opt=canceltask&token=%s&taskExecuteId=%s" % (host,token,taskExecuteId)
            return_msg['gettaskprogressUrl'] = "HTTP://%s/invoke?opt=gettaskprogress&token=%s&taskExecuteId=%s" % (host,token,taskExecuteId)
...
def callbackinterface(paramDict):
...
def getInterfaceResult(token,interfaceDebugId,execTimeout):
    ...
    while True:
        ...
        if startTimer > execTimeout :
            return_msg = {}
            return_msg['callbackinterfaceUrl'] = "HTTP://%s/invoke?opt=callbackinterface&token=%s&interfaceDebugId=%s" % (host,token,interfaceDebugId)
...
def callbacktestcase(paramDict):
...
def getCaseResult(token,testCaseDebugId,testCaseStepDebugId,execTimeout):
    ...
    while True:
        ...
        if startTimer > execTimeout:
            return_msg = {}
            return_msg['callbackTestcaseUrl'] = "HTTP://%s/invoke?opt=callbacktestcase&token=%s&testCaseDebugId=%s&testCaseStepDebugId=%s" % (host, token, testCaseDebugId,testCaseStepDebugId)
...
def getuiTaskResult(token,taskExecuteId,timeout):
    ...
    while True:
        ...
        if startTimer > timeout :
            return_msg = {}
            return_msg['callbacktaskUrl'] = "HTTP://%s/invoke?opt=callbackuitask&token=%s&taskExecuteId=%s" % (host,token,taskExecuteId)
            return_msg['canceltaskUrl'] = "HTTP://%s/invoke?opt=canceluitask&token=%s&taskExecuteId=%s" % (host,token,taskExecuteId)
            return HttpResponse(ApiReturn(ApiReturn.CODE_TIMEOUT, langDict['invoke']['taskexecutetimeout'],return_msg).toJson())
...
def callbackdubbotask(paramDict):
    taskExecuteId = paramDict.get("taskExecuteId")
    if isInt(taskExecuteId) == False or int(taskExecuteId) < 0 :
        return HttpResponse(ApiReturn(ApiReturn.CODE_PARAM_ERROR, langDict['invoke']['taskexecidmustbelagerthan0'], retMsg).toJson())
    else:
...
def getdubbotaskprogress(paramDict):
    taskExecuteId = paramDict.get("taskExecuteId")
    if isInt(taskExecuteId) == False or int(taskExecuteId) < 0 :
        return HttpResponse(ApiReturn(ApiReturn.CODE_PARAM_ERROR, langDict['invoke']['taskexecidmustbelagerthan0'], retMsg).toJson())
    else:
...
def getdubboTaskResult(token,taskExecuteId,timeout):
    ...
    while True:
        ...
        if startTimer > timeout :
            return_msg = {}
            return_msg['callbacktaskUrl'] = "HTTP://%s/invoke?opt=callbackdubbotask&token=%s&taskExecuteId=%s" % (host,token,taskExecuteId)
            return_msg['canceltaskUrl'] = "HTTP://%s/invoke?opt=canceldubbotask&token=%s&taskExecuteId=%s" % (host,token,taskExecuteId)
            return_msg['gettaskprogressUrl'] = "HTTP://%s/invoke?opt=getdubbotaskprogress&token=%s&taskExecuteId=%s" % (host,token,taskExecuteId)
            return HttpResponse(ApiReturn(ApiReturn.CODE_TIMEOUT, langDict['invoke']['taskexecutetimeout'],return_msg).toJson())
...
def addStatisticTaskExecuteInfo(paramDict):
...
def getTaskSuiteResult(token,taskSuiteExecuteId,timeout):
    ...
    while True:
        ...
        if startTimer > timeout :
            return_msg = {}
            return_msg['callbacktaskUrl'] = "HTTP://%s/invoke?opt=callbacktasksuite&token=%s&taskSuiteExecuteId=%s" % (host,token,taskSuiteExecuteId)
...
def callbacktasksuite(paramDict):
...
def getDubboTaskSuiteResult(token,taskSuiteExecuteId,timeout):
    ...
    while True:
        ...
        if startTimer > timeout :
            return_msg = {}
            return_msg['callbacktaskUrl'] = "HTTP://%s/invoke?opt=callbackdubbotasksuite&token=%s&taskSuiteExecuteId=%s" % (host,token,taskSuiteExecuteId)
            # return_msg['canceltaskUrl'] = "HTTP://%s/invoke?opt=canceldubbotask&token=%s&taskExecuteId=%s" % (host,token,taskSuiteExecuteId)
            # return_msg['gettaskprogressUrl'] = "HTTP://%s/invoke?opt=getdubbotaskprogress&token=%s&taskExecuteId=%s" % (host,token,taskSuiteExecuteId)
            return HttpResponse(ApiReturn(ApiReturn.CODE_TIMEOUT, langDict['invoke']['taskexecutetimeout'],return_msg).toJson())
...
def callbackdubbotasksuite(paramDict):
    taskSuiteExecuteId = paramDict.get("taskSuiteExecuteId")
    if isInt(taskSuiteExecuteId) == False or int(taskSuiteExecuteId) < 0 :
        return HttpResponse(ApiReturn(ApiReturn.CODE_PARAM_ERROR, langDict['invoke']['tasksuiteexecidmustbelagerthan0'], retMsg).toJson())
    else:
...
def canceldubbotasksuite(paramDict):
    taskSuiteExecuteId = paramDict.get("taskSuiteExecuteId")
    if isInt(taskSuiteExecuteId) == False or int(taskSuiteExecuteId) < 0:
        return HttpResponse(
            ApiReturn(ApiReturn.CODE_PARAM_ERROR, langDict['invoke']['taskexecidmustbelagerthan0'], retMsg).toJson())

...
def mock(request,service,tagKey,httpConfKey,url):
    ...
    if retBl == False:
        ...
        tcResp = HttpProcesserV2(method = request.method.upper(),host = reqHost,url = url,headers = json.dumps(processedReqHeader),params = djangoReqGetDictToParamStr(request.GET),bodyType = bodyType,bodyContent=bodyContent).sendRequest()
